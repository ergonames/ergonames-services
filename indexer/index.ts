...
import {
  getInitialRegistryCreationInformation,
...
async function main() {
  ...
  const registryCreationTransactionId =
    ergonamesInformation.registryCreationTransactionId;
  ...
  let irci: RegistryCreationInformation =
    await getInitialRegistryCreationInformation(
      registryCreationTransactionId,
      explorerUrl,
    );
  // console.log(irci);

  let ri: RegistrationInformation = await getRegistryInformation(
    irci.spentTransactionId,
    explorerUrl,
  );
  console.log(ri);
